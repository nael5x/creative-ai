...
export function App() {
  ...
  return <>
    <header className="site-header"><div className="page-width header-inner"><a className="brand" href="#compare" aria-label="Creative AI">CREATIVE <span>AI</span></a><nav aria-label="Primary"><a href="#compare">{copy.compare}</a><a href="#directory">{copy.directory}</a><a href="#updates">{copy.updates}</a><a href="#methodology">{copy.methodology}</a></nav><button className="language" onClick={() => setLanguage((value) => value === "en" ? "ar" : "en")}>◎ {language === "en" ? "العربية" : "English"}</button></div></header>
    ...
    <Directory language={language} copy={copy} onCompare={(right) => { const left = right === state.left ? state.right : state.left; update({ left, right }); document.getElementById("compare")?.scrollIntoView(); }} />
    <Updates copy={copy} />
    ...
    <footer><div className="page-width"><strong>CREATIVE <span>AI</span></strong><p>{copy.disclosure}</p></div></footer>
    {advisorOpen ? <Advisor copy={copy} onClose={() => setAdvisorOpen(false)} onChoose={(left, right, mode) => { update({ left, right, mode }); setAdvisorOpen(false); }} /> : null}
  </>;
}
